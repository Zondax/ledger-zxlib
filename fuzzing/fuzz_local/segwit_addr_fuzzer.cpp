...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    if (encode_result) {
        ...
        const int decode_result = segwit_addr_decode(&decode_ver, decode_prog, &decode_prog_len, 
                                              hrp, encode_output);
        (void)decode_result;
    }
    ...
    if (bech32_result) {
        ...
        const bech32_encoding decode_enc = bech32_decode(decode_hrp, decode_data, &decode_data_len, 
                                                  bech32_output);
        (void)decode_enc;
    }
