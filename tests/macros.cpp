...
namespace {
    ...
    TEST(UINT64_TO_STR, Zero) {

        char temp[21];
        const char *error = uint64_to_str(temp, sizeof(temp), uint64_t(0));
        EXPECT_STREQ(temp, "0");
        EXPECT_TRUE(error == nullptr);
    }

    TEST(UINT64_TO_STR, Positive_1234) {

        char temp[21];
        const char *error = uint64_to_str(temp, sizeof(temp), uint64_t(1234));
        EXPECT_STREQ(temp, "1234");
        EXPECT_TRUE(error == nullptr);
    }

    TEST(UINT64_TO_STR, TooSmall_0) {

        char temp[1];
        const char *error = uint64_to_str(temp, sizeof(temp), int64_t(0));
        EXPECT_STREQ("Buffer too small", error);
    }

    TEST(UINT64_TO_STR, FitsJust) {
        char temp[4];
        const char *error = uint64_to_str(temp, sizeof(temp), uint64_t(999));
        EXPECT_STREQ(temp, "999");
        EXPECT_TRUE(error == nullptr);
    }

    TEST(UINT64_TO_STR, TooSmall_10) {
        char temp[2];
        const char *error = uint64_to_str(temp, sizeof(temp), uint64_t(10));
        EXPECT_STREQ("Buffer too small", error);
    }

    TEST(UINT64_TO_STR, Max) {
        char temp[21];
        const char *error = uint64_to_str(temp, sizeof(temp), std::numeric_limits<uint64_t>::max());
        EXPECT_STREQ(temp, "18446744073709551615");
        EXPECT_TRUE(error == nullptr);
    }
