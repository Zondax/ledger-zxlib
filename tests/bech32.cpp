...
namespace {
    ...
    TEST(BECH32, bip173_test_vectors) {
        ...
        // Note: bech32EncodeFromBytes does raw bech32 encoding, not segwit address encoding
        {
            ...
            auto err = bech32EncodeFromBytes(addr_out, sizeof(addr_out), "bc", data, sizeof(data), 1, BECH32_ENCODING_BECH32);
            ASSERT_EQ(err, zxerr_ok);
        ...
        {
            uint8_t data[] = {0x00};
            auto err = bech32EncodeFromBytes(addr_out, sizeof(addr_out), "a", data, sizeof(data), 1, BECH32_ENCODING_BECH32);
            ASSERT_EQ(err, zxerr_ok);
        ...
        // Test case 4: Verify HRP and separator
        {
            ...
            auto err = bech32EncodeFromBytes(addr_out, sizeof(addr_out), "test", data, sizeof(data), 1, BECH32_ENCODING_BECH32);
            ASSERT_EQ(err, zxerr_ok);
    ...
    TEST(BECH32, edge_cases_buffer_boundaries) {
        ...
        // Test case 2: 40 bytes data
        {
            ...
            auto err = bech32EncodeFromBytes(addr_out, sizeof(addr_out), "bc", data, sizeof(data), 0, BECH32_ENCODING_BECH32);
            ASSERT_EQ(err, zxerr_ok);
        ...
        // Test case 3: Exactly fitting output buffer
        {
            ...
            auto err = bech32EncodeFromBytes(small_out, sizeof(small_out), "bc", data, sizeof(data), 0, BECH32_ENCODING_BECH32);
    ...
    TEST(BECH32, different_hrp_lengths) {
        ...
        {
            auto err = bech32EncodeFromBytes(addr_out, sizeof(addr_out), "a", data, sizeof(data), 0, BECH32_ENCODING_BECH32);
            ASSERT_EQ(err, zxerr_ok);
        ...
        // Test case 2: Maximum reasonable HRP (83 chars is the limit)
        {
            ...
            auto err = bech32EncodeFromBytes(addr_out, sizeof(addr_out), long_hrp, data, sizeof(data), 0, BECH32_ENCODING_BECH32);
        ...
        // Test case 3: Common cryptocurrency HRPs
        {
            ...
            for (const auto& hrp : hrps) {
                auto err = bech32EncodeFromBytes(addr_out, sizeof(addr_out), hrp, data, sizeof(data), 0, BECH32_ENCODING_BECH32);
                ASSERT_EQ(err, zxerr_ok);
    ...
    TEST(BECH32, bech32_vs_bech32m_comparison) {
        ...
        // Encode with BECH32
        auto err1 = bech32EncodeFromBytes(addr_out_bech32, sizeof(addr_out_bech32), "bc", data, sizeof(data), 0, BECH32_ENCODING_BECH32);
        ASSERT_EQ(err1, zxerr_ok);
        
        // Encode with BECH32M
        auto err2 = bech32EncodeFromBytes(addr_out_bech32m, sizeof(addr_out_bech32m), "bc", data, sizeof(data), 0, BECH32_ENCODING_BECH32M);
    ...
    TEST(BECH32, verified_hex_test_vectors) {
        ...
        // 4-byte hex with bc HRP, raw data (pad=1)
        {
            ...
            auto err = bech32EncodeFromBytes(addr_out, sizeof(addr_out), "bc", data_buffer, data_len, 1, BECH32_ENCODING_BECH32);
            ASSERT_EQ(err, zxerr_ok);
        ...
        // 1-byte hex with test HRP, raw data (pad=1)
        {
            ...
            uint32_t data_len = hexstr_to_array(data_buffer, sizeof(data_buffer), hex_input, strlen(hex_input));
            auto err = bech32EncodeFromBytes(addr_out, sizeof(addr_out), "test", data_buffer, data_len, 1, BECH32_ENCODING_BECH32);
            ASSERT_EQ(err, zxerr_ok);
            ASSERT_STREQ(expected, addr_out);
        }
        ...
        // Zero byte with minimal HRP, raw data (pad=1)
        {
            ...
            uint32_t data_len = hexstr_to_array(data_buffer, sizeof(data_buffer), hex_input, strlen(hex_input));
            auto err = bech32EncodeFromBytes(addr_out, sizeof(addr_out), "a", data_buffer, data_len, 1, BECH32_ENCODING_BECH32);
            ASSERT_EQ(err, zxerr_ok);
            ASSERT_STREQ(expected, addr_out);
        }
    ...
    TEST(BECH32M, verified_hex_test_vectors) {
        ...
        // 4-byte hex with bc HRP, raw data (pad=1, bech32m)
        {
            ...
            uint32_t data_len = hexstr_to_array(data_buffer, sizeof(data_buffer), hex_input, strlen(hex_input));
            auto err = bech32EncodeFromBytes(addr_out, sizeof(addr_out), "bc", data_buffer, data_len, 1, BECH32_ENCODING_BECH32M);
            ASSERT_EQ(err, zxerr_ok);
            ASSERT_STREQ(expected, addr_out);
        }
        ...
        // 1-byte hex with test HRP, raw data (pad=1, bech32m)
        {
            ...
            uint32_t data_len = hexstr_to_array(data_buffer, sizeof(data_buffer), hex_input, strlen(hex_input));
            auto err = bech32EncodeFromBytes(addr_out, sizeof(addr_out), "test", data_buffer, data_len, 1, BECH32_ENCODING_BECH32M);
            ASSERT_EQ(err, zxerr_ok);
            ASSERT_STREQ(expected, addr_out);
        }
    ...
    TEST(BECH32, pad_zero_test_vectors) {
        ...
        // 8-byte pattern with pad=0 - should produce different result than pad=1
        {
            ...
            uint32_t data_len = hexstr_to_array(data_buffer, sizeof(data_buffer), hex_input, strlen(hex_input));
            auto err = bech32EncodeFromBytes(addr_out, sizeof(addr_out), "test", data_buffer, data_len, 0, BECH32_ENCODING_BECH32);
            ASSERT_EQ(err, zxerr_ok);
            ASSERT_STREQ(expected, addr_out);
        }
        ...
        // 5-byte pattern with pad=0 (5*8=40 bits, divisible by 5)
        {
            ...
            uint32_t data_len = hexstr_to_array(data_buffer, sizeof(data_buffer), hex_input, strlen(hex_input));
            auto err = bech32EncodeFromBytes(addr_out, sizeof(addr_out), "bc", data_buffer, data_len, 0, BECH32_ENCODING_BECH32);
            ASSERT_EQ(err, zxerr_ok);
            ASSERT_STREQ(expected, addr_out);
        }
    }
    ...
    TEST(BECH32, additional_comprehensive_cases) {
        ...
        // Test 5: Different HRP (testnet)
        {
            ...
            uint32_t data_len = hexstr_to_array(data_buffer, sizeof(data_buffer), hex_input, strlen(hex_input));
            auto err = bech32EncodeFromBytes(addr_out, sizeof(addr_out), "tb", data_buffer, data_len, 1, BECH32_ENCODING_BECH32);
            ASSERT_EQ(err, zxerr_ok);
            ASSERT_STREQ(expected, addr_out);
        }
        ...
        // Test 7: Bech32m vs Bech32 comparison (same input, different encoding)
        {
            ...
            err = bech32EncodeFromBytes(addr_out, sizeof(addr_out), "bc", data_buffer, data_len, 1, BECH32_ENCODING_BECH32M);
            ASSERT_EQ(err, zxerr_ok);
        ...
        // Test 8: fuzzing error 
        {
            ...
            // Test bech32
            auto err = bech32EncodeFromBytes(addr_out, sizeof(addr_out), "coston", data_buffer, data_len, 1, BECH32_ENCODING_BECH32);
            ASSERT_EQ(err, zxerr_ok);
            ASSERT_STREQ(expected_bech32, addr_out);
            
            // Test bech32m - should produce different checksum
            err = bech32EncodeFromBytes(addr_out, sizeof(addr_out), "coston", data_buffer, data_len, 1, BECH32_ENCODING_BECH32M);
            ASSERT_EQ(err, zxerr_ok);
            ASSERT_STREQ(expected_bech32m, addr_out);
        }
        ...
        // Test 14: Pi digits pattern with cosmos HRP and bech32m encoding
        {
            ...
            uint32_t data_len = hexstr_to_array(data_buffer, sizeof(data_buffer), hex_input, strlen(hex_input));
            auto err = bech32EncodeFromBytes(addr_out, sizeof(addr_out), "cosmos", data_buffer, data_len, 1, BECH32_ENCODING_BECH32M);
            ASSERT_EQ(err, zxerr_ok);
            ASSERT_STREQ(expected, addr_out);
        }
        ...
        // Test 15: Litecoin HRP with 0x55 pattern (6 bytes)
        {
            ...
            uint32_t data_len = hexstr_to_array(data_buffer, sizeof(data_buffer), hex_input, strlen(hex_input));
            auto err = bech32EncodeFromBytes(addr_out, sizeof(addr_out), "ltc", data_buffer, data_len, 1, BECH32_ENCODING_BECH32);
            ASSERT_EQ(err, zxerr_ok);
            ASSERT_STREQ(expected, addr_out);
        }
