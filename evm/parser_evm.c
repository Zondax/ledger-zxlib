...
static parser_evm_error_t parser_evm_init_context(parser_evm_context_t *ctx, const uint8_t *buffer, uint16_t bufferSize) {
    ctx->offset = 0;
    ctx->buffer = NULL;
    ctx->bufferLen = 0;
...
parser_evm_error_t parser_getItemEth(const parser_evm_context_t *ctx, uint8_t displayIdx, char *outKey, uint16_t outKeyLen,
...
parser_evm_error_t parser_compute_eth_v(parser_evm_context_t *ctx, unsigned int info, uint8_t *v, bool is_personal_message) {
