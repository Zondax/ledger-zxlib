...
parser_evm_error_t rlp_parseStream(parser_evm_context_t *ctx, rlp_t *rlp, uint16_t *fields, uint16_t maxFields) {
    ...
    *fields = 0;
    while (ctx->offset < ctx->bufferLen && (*fields) < maxFields) {
        CHECK_EVM_ERROR(rlp_read(ctx, rlp))
        (*fields)++;
        rlp++;
    }
...
static parser_evm_error_t readBytes(parser_evm_context_t *ctx, uint8_t const **buff, uint64_t buffLen) {
...
parser_evm_error_t rlp_read(parser_evm_context_t *ctx, rlp_t *rlp) {
    ...
    CHECK_EVM_ERROR(readBytes(ctx, &prefixPtr, 1))
    ...
    if (prefix <= RLP_KIND_BYTE_PREFIX) {
        ...
        if (rlp->rlpLen == 0) {
            ...
            CHECK_EVM_ERROR(readBytes(ctx, &rlp->ptr, rlp->rlpLen))
        }
        ...
        CHECK_EVM_ERROR(readBytes(ctx, &rlpLenPtr, bytesLen))
        rlp->rlpLen = 0;
        for (uint8_t i = 0; i < bytesLen; i++) {
            // Check for overflow before shifting
            if (rlp->rlpLen > (UINT64_MAX >> 8)) {
                return parser_evm_value_out_of_range;
            }
            rlp->rlpLen <<= 8u;
...
parser_evm_error_t rlp_readList(const rlp_t *list, rlp_t *fields, uint16_t *listFields, uint16_t maxFields) {
    ...
    parser_evm_context_t ctx = {.buffer = list->ptr, .bufferLen = list->rlpLen, .offset = 0};
...
parser_evm_error_t rlp_readUInt256(const rlp_t *rlp, uint256_t *value) {
    ...
    switch (rlp->kind) {
        case RLP_KIND_STRING:
            if (rlp->rlpLen > sizeof(tmpBuffer)) return parser_evm_value_out_of_range;
        ...
        default:
            return parser_evm_unexpected_type;
    }

    parser_evm_context_t ctx = {.buffer = tmpBuffer, .bufferLen = sizeof(tmpBuffer), .offset = 0};
    CHECK_EVM_ERROR(readu256BE(&ctx, value));
    return parser_evm_ok;
}
