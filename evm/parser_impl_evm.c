 ...
 ********************************************************************************/
 ...
 extern parser_evm_error_t getNumItemsEthAppSpecific(eth_tx_t *ethTxObj, uint8_t *numItems);
 extern parser_evm_error_t printGenericAppSpecific(const parser_evm_context_t *ctx, const eth_tx_t *ethTxObj, uint8_t displayIdx, char *outKey,
                                               uint16_t outKeyLen, char *outVal, uint16_t outValLen, uint8_t pageIdx,
                                               uint8_t *pageCount);
 extern parser_evm_error_t printERC20TransferAppSpecific(const parser_evm_context_t *ctx, const eth_tx_t *ethTxObj, uint8_t displayIdx, char *outKey,
 ...
 static parser_evm_error_t readChainID(parser_evm_context_t *ctx, rlp_t *chainId) {
     if (ctx == NULL || chainId == NULL) {
         return parser_evm_unexpected_error;
     }
 
     CHECK_EVM_ERROR(rlp_read(ctx, chainId));
     uint64_t tmpChainId = 0;
     if (chainId->rlpLen > 1) {
         CHECK_EVM_ERROR(be_bytes_to_u64(chainId->ptr, (uint8_t)chainId->rlpLen, &tmpChainId))
     ...
     if (supported_networks_evm_len == 0) {
         return parser_evm_invalid_chain_id;
     }
 ...
 static parser_evm_error_t parse_legacy_tx(parser_evm_context_t *ctx, eth_tx_t *tx_obj) {
     if (ctx == NULL || tx_obj == NULL) {
         return parser_evm_unexpected_error;
     }
 
     CHECK_EVM_ERROR(rlp_read(ctx, &tx_obj->tx.nonce));
     CHECK_EVM_ERROR(rlp_read(ctx, &(tx_obj->tx.gasPrice)));
     CHECK_EVM_ERROR(rlp_read(ctx, &(tx_obj->tx.gasLimit)));
     CHECK_EVM_ERROR(rlp_read(ctx, &(tx_obj->tx.to)));
     CHECK_EVM_ERROR(rlp_read(ctx, &(tx_obj->tx.value)));
     CHECK_EVM_ERROR(rlp_read(ctx, &(tx_obj->tx.data)));
     ...
     CHECK_EVM_ERROR(readChainID(ctx, &tx_obj->chainId));
 
     // Check R and S fields
     rlp_t sig_r = {0};
     CHECK_EVM_ERROR(rlp_read(ctx, &sig_r));
 
     rlp_t sig_s = {0};
     CHECK_EVM_ERROR(rlp_read(ctx, &sig_s));
     ...
     return parser_evm_invalid_rs_values;
 }
 
 static parser_evm_error_t parse_2930(parser_evm_context_t *ctx, eth_tx_t *tx_obj) {
     if (ctx == NULL || tx_obj == NULL) {
         return parser_evm_unexpected_error;
     }
     ...
     CHECK_EVM_ERROR(rlp_read(ctx, &(tx_obj->tx.value)));
     CHECK_EVM_ERROR(rlp_read(ctx, &(tx_obj->tx.data)));
     CHECK_EVM_ERROR(rlp_read(ctx, &(tx_obj->tx.access_list)));
 
     // R and S fields should be empty
     if (ctx->offset < ctx->bufferLen) {
         return parser_evm_unsupported_tx;
     }
 
     return parser_evm_ok;
 }
 
 static parser_evm_error_t parse_1559(parser_evm_context_t *ctx, eth_tx_t *tx_obj) {
     if (ctx == NULL || tx_obj == NULL) {
         return parser_evm_unexpected_error;
     }
     ...
     CHECK_EVM_ERROR(readChainID(ctx, &tx_obj->chainId));
     CHECK_EVM_ERROR(rlp_read(ctx, &tx_obj->tx.nonce));
     CHECK_EVM_ERROR(rlp_read(ctx, &(tx_obj->tx.max_priority_fee_per_gas)));
     CHECK_EVM_ERROR(rlp_read(ctx, &(tx_obj->tx.max_fee_per_gas)));
     CHECK_EVM_ERROR(rlp_read(ctx, &(tx_obj->tx.gasLimit)));
     CHECK_EVM_ERROR(rlp_read(ctx, &(tx_obj->tx.to)));
     CHECK_EVM_ERROR(rlp_read(ctx, &(tx_obj->tx.value)));
     CHECK_EVM_ERROR(rlp_read(ctx, &(tx_obj->tx.data)));
 ...
 static parser_evm_error_t readTxnType(parser_evm_context_t *ctx, eth_tx_type_e *type) {
 ...
 parser_evm_error_t _readEth(parser_evm_context_t *ctx, eth_tx_t *tx_obj) {
     if (ctx == NULL || tx_obj == NULL) {
         return parser_evm_unexpected_value;
     }
 
     MEMZERO(&eth_tx_obj, sizeof(eth_tx_obj));
     CHECK_EVM_ERROR(readTxnType(ctx, &tx_obj->tx_type))
     // We expect a list with all the fields from the transaction
     rlp_t list = {0};
     CHECK_EVM_ERROR(rlp_read(ctx, &list));
     ...
     parser_evm_context_t txCtx = {.buffer = list.ptr, .bufferLen = list.rlpLen, .offset = 0};
 ...
 parser_evm_error_t _validateTxEth() {
     ...
     if (eth_tx_obj.tx.data.rlpLen == 0 || validateERC20(&eth_tx_obj)) {
         ...
         return parser_evm_blindsign_mode_required;
     }
 
     return parser_evm_ok;
 }
 
 static parser_evm_error_t printERC20Transfer(const parser_evm_context_t *ctx, uint8_t displayIdx, char *outKey, uint16_t outKeyLen, char *outVal,
                                          uint16_t outValLen, uint8_t pageIdx, uint8_t *pageCount) {
 ...
 static parser_evm_error_t printGeneric(const parser_evm_context_t *ctx, uint8_t displayIdx, char *outKey, uint16_t outKeyLen,
